...
def gen_3ch_histogram(src, ch_id, col_histo):
    histo = cv.calcHist([src], [ch_id], None, [256], [0, 256])
    ...
    for i in range (1, 256):
        ...
        cv.line(histoImg, (x, y), (px, py), col_histo, 1, cv.LINE_AA)
        px, py = x, y
...
while True:
    ...
    img_l = cv.vconcat([img, img_sp])
    img_r = cv.vconcat([histo_r, histo_g, histo_b])
    frame = cv.hconcat([img_l, img_r])

    tm.stop()
    disp_fps = f"{tm.getTimeMilli():.2f}"
    tm.reset()
    tm.start()
    cv.putText(frame, disp_fps, (10, 50), cv.FONT_HERSHEY_PLAIN, 2, (0, 0, 255), 1, cv.LINE_AA)

    cv.imshow(w_name, frame)
    
    key = cv.waitKey(1) # キー入力を1ms待って、k が27（ESC）だったらBreakする
    if key == 27:
        break
